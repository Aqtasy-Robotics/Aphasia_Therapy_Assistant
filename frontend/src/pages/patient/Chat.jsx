  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      const { data, error } = await supabase
        .from('messages')
        .select('*')
        .or(`and(sender_id.eq.${myId},receiver_id.eq.${therapist.id}),and(sender_id.eq.${therapist.id},receiver_id.eq.${myId})`)
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-160px)] flex flex-col items-center justify-center bg-white rounded-[3.5rem] shadow-2xl shadow-gray-200/40 font-sans antialiased">
        ...
        <span className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-gray-400">Loading Secure Chat...</span>
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100vh-160px)] flex bg-white rounded-[3.5rem] overflow-hidden border border-gray-50 shadow-2xl shadow-gray-200/40 font-sans antialiased">
      ...
      <aside className="w-80 bg-gray-50/50 border-r border-gray-100 flex flex-col relative shrink-0">
        ...
        <div className="p-8">
          <h2 className="text-2xl font-extrabold text-gray-800 tracking-tight">Messages</h2>
        ...
        {therapist ? (
          <div className="p-4">
            <div className="p-5 bg-white border border-[#172554]/10 rounded-[2rem] shadow-xl shadow-[#172554]/5 flex flex-col items-center text-center relative overflow-hidden group">
              ...
              <h3 className="font-extrabold text-gray-800 text-sm tracking-tight">{therapist.full_name}</h3>
              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">Primary Therapist</p>
        ...
        {!therapist ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-gray-400 max-w-sm mt-5 font-semibold leading-relaxed text-sm italic">
              This is your secure clinical space. Once your therapist sets up your account, you will be able to message them directly here.
            </p>
          </div>
        ) : (
          /* ACTIVE CHAT VIEW */
          <>
            ...
            <div className="p-6 border-b border-gray-50 bg-white/80 backdrop-blur-md flex justify-between items-center z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-extrabold text-gray-800 tracking-tight">{therapist.full_name}</h3>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <ShieldCheck className="w-3 h-3 text-[#064e3b]" />
                    <p className="text-[9px] font-bold text-[#064e3b] uppercase tracking-widest">Secure Chat</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-gray-50/10 custom-scrollbar">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                  ...
                  <p className="text-xs font-bold text-gray-500">Secure connection established.</p>
                  <p className="text-[10px] text-gray-400 mt-1">Send a message to your therapist to begin.</p>
                </div>
              ) : (
                messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[75%] p-5 rounded-[2rem] shadow-sm ${
                        ...
                        <p className={`text-[9px] font-bold mt-3 text-right ${isMine ? "text-white/50" : "text-gray-400"}`}>
            ...
            <div className="p-8 border-t border-gray-50 bg-white">
              <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex items-center gap-5 bg-gray-50 p-3 rounded-[2.5rem] border border-gray-100 focus-within:ring-4 focus-within:ring-[#172554]/5 transition-all shadow-sm">
                <button type="button" className="p-3 text-gray-400 hover:text-[#172554] transition-colors">
